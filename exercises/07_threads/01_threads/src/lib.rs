...
pub fn sum(v: Vec<i32>) -> i32 {
    ...
    let left = left.to_vec(); // need to cast to vec to avoid borrowing issues (&[i32] ==> Vec<i32>)
    let right = right.to_vec(); // need to cast to vec to avoid borrowing issues (&[i32] ==> Vec<i32>)
